...
function bbp_list_forums( $args = '' ) {
	...
	if ( !empty( $sub_forums ) ) {
		...
		foreach ( $sub_forums as $sub_forum ) {
			...
			// $output .= bbp_get_template_part( 'loop', 'single-topic' );
...
/**
 *   CUSTOM list of forums 
 */
function bbp_list_forums_custom( $args = '' ) {
	...
	// Parse arguments against default values
	$r = bbp_parse_args( $args, array(
		'before'            => '<ul class="bbp-forums-list_custom">',
		'after'             => '</ul>',
		'link_before'       => '<li class="bbp-forum">',
		'link_after'        => '</li>',
		'count_before'      => ' (',
	...
	if ( !empty( $sub_forums ) ) {
		...
		foreach ( $sub_forums as $sub_forum ) {
			...
			// Build this sub forums link
			$output .= "<ul class='bbp-forums-list_custom test'>";
			$output .= $r['link_before'] . '<a href="' . esc_url( $permalink ) . '" class="bbp-forum-link">' . $title .  '</a>' . $show_sep . $r['link_after'];

			$output .= "<li class='bbp_count_custom'>".  $count['topic']."</li>";
			$output .= "<li class='bbp_count_custom'>".  $count['reply']."</li>";

			$obj = $sub_forum;
			$subID =  $obj->ID;
			$last_active = bbp_get_forum_last_active_id($subID);
			$lastactive_time = bbp_get_forum_last_active_time($subID);
			...
			$output .= "<li class='bbp-topic-freshness-custom'>". $meta."</li>";
			$output .= " </ul>";
		...
		// Output the list
		// echo apply_filters( 'bbp_list_forums_custom', $r['before'] . $output . $r['after'], $r );
		echo apply_filters( 'bbp_list_forums_custom',  $output , $r );
	}
}
