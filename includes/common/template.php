...
function bbp_breadcrumb( $args = array() ) {
	...
	function bbp_get_breadcrumb( $args = array() ) {
		...
		if ( !empty( $r['include_root'] ) || empty( $r['root_text'] ) ) {
			...
			if ($r['current_text'] == "Forum"){
				$crumbs[] = '<span class="root_text">'.$r['root_text'].'</span>';
			} else {
		...
		if ( !empty( $r['include_current'] ) || empty( $r['current_text'] ) ) {
			...
			if ($r['current_text'] == "Forum"){
				$crumbs[] = $r['current_before'] . "" . $r['current_after'];
			} else {
		...
		// Build the trail
		$trail  = !empty( $crumbs ) ? ( $r['before'] ."YOU ARE HERE  : " .  $r['crumb_before'] . implode( $sep . $r['crumb_after'] . $r['crumb_before'] , $crumbs ) . $r['crumb_after'] . $r['after'] ) : '';
